...
// CODE HERE
multiply = (num1, num2, tree) => tree(num1*num2)
...
// CODE HERE 
first = (arr, callback) => callback(arr[0])
...
// CODE HERE
newNames = uniqArr => console.log(`The new names array with all the duplicate items removed is ${uniqArr}.`)
uniq(names, newNames)
...
// CODE HERE
each(names, itemIndex = (item, index) => console.log(`The item at index ${index} is ${item}.`))
...
// CODE HERE 
getUserById = (arr, idNum, callback) => {
  for(let i=0; i<arr.length; i++){
    if(idNum===arr[i].id){
      console.log('ifed')
      callback(arr[i])
