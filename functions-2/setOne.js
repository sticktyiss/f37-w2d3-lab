...
const fGrape = function findGrape (arr) {
    for(let i=0; i<arr.length; i++) {
        if(arr[i].color === "purple") {
            return console.log(`The fruit with index ${arr.indexOf(arr[i])} is a grape`)
        } 
...
// CODE HERE
func = (para1, para2) => console.log(`The first parameter is ${para1}. The second parameter is ${para2}.`)
...
greeting = (firstName, lastName, tree) => {
    return tree(firstName + lastName)
}
    // return fullName('1'+'Goodman')

fullName = me => console.log(`Hello, my full name is ${me}`)
// INVOKE 'GREETING FUNCTION' HERE
greeting('1', 'Goodman', fullName)
...
// CODE HERE
pricesPlusTax = (arr, tree) => {
    for(let i=0; i<arr.length; i++){
        let newCost = arr[i]*1.20
...
// CODE HERE
log = () => console.log(`The new array plus tax = ${totalCost}`)
pricesPlusTax(prices, log)
...
function multiplyingFactory(num) {
    return function innerFunction(num1){
        if(num >= 5){
...
multiplyingFactory = num => {
    return function inFunc(num1){
    num>=5 ? console.log(num*num1) : console.log('Cannot multiply: the first number is smaller than 5.')
    }
}
/* 
